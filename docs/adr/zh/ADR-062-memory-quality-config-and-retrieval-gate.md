...
> ⏸ **M5 计划待用户确认**——本文档 §6.5（auto_inject 触发策略：方案 A 首轮触发）/ §6.6（keyword_index 方案：写时拼入 object）/ §6.7（后续待办）已写明，**代码未动**。
>
> - 当前代码：auto_inject 默认 `false`、无 `quality.keyword_index` 入口；M1-M3.6/M4 已全绿落地。
> - 提交历史：`15654af0`（含 M5 草案的安全快照）→ `b117f901`（仅撤回 M5 代码增量）。
> - 待办：用户审查 §6.5-6.7 后告知是否按计划实施，或需调整。
