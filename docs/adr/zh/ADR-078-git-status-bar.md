...
| 2 | git 引擎 | **系统 git CLI**（`git status --porcelain=v1 -z` / `git show HEAD:<path>` / `git log`），不引入 git2 / gix 编译依赖；`tokio::process::Command` + 超时 + 输出上限；git 缺失时显式报错 |
...
| 4 | Desktop 数据层 | 新 `gitStore.ts`（仿 fileTreeStore：SWR + 缓存 + agent/workspace 切换失效 + `with503Retry`），并订阅 fs-changed 事件做自动刷新 |
...
1. **git 操作只读**：三个 API 不得改动工作树、.git、index 或 refs（porcelain status 不触发写；`git show` 是纯读；不启用外部 diff 工具）。
...
7. **自动刷新**：面板展开时订阅 fs-watch（根路径），折叠时取消——与工作树原理一致（ADR-058 demand-driven）。
...
**执行规范**（关键，防注入/防异常）：
- `tokio::process::Command`，**不拼 shell 字符串**；repo root / path 均以 `Command::arg` 传递；路径参数用 `--` 分隔符保护以 `-` 开头的名字。
...
- 请求 path 必须相对 workspace root；Runtime 先转绝对、校验、再 strip 回相对传给 git。
...
```json
200 {
  "kind": "modified" | "untracked" | "binary" | "no_change",
  "original": "…",      // HEAD 版全文；untracked 为 ""；staged(cached=1) 时本字段为 HEAD，modified 为 index
  "modified": "…"       // 工作树全文（cached=1 时为 index 全文 git show :<path>）
}
```
- cached=0（默认，Show Diff）：original = `git show HEAD:<path>`，modified = 工作树文件内容（复用 Runtime 现有 `/workspaces/file` 读取逻辑，保证与工作树一致）。
- cached=1（Staged Diff，预留）：original = HEAD，modified = `git show :<path>`（index）。
- untracked：`original=""`、`modified=文件内容`（DiffEditor 呈现全新增）；binary：`kind="binary"` 不返回内容（DiffEditor 显示占位）；无差异 → `kind="no_change"`。
...
- Desktop 新增 `stores/gitStore.ts`：仿 `fileTreeStore`——SWR 缓存 + 去重 + `with503Retry`；`fetchStatus(agentId, workspaceId)`、`fetchDiff(...)`、`fetchLog(...)`；`invalidate(agentId, workspaceId)`（切换 agent/workspace 时清空）；`refresh()` 强制直取。另注册 fs-changed 事件处理器（见决策 7）。
...
- 行点击 → `fileEditorStore.openFile(agentId, workspaceId, path)`；行右键 → `ContextMenu`（tab 右键同款组件），菜单项：Show Diff / Show Log / 在编辑器中打开 / 复制路径。
...
- 文件 id 约定 `git:diff:<workspaceId>:<path>` / `git:log:<workspaceId>:<path>`，保证 tab 唯一可切换。
...
- **自动刷新**：gitStore 注册 `workspaceFsEvents` 的 fs-changed 处理器，命中当前 (agent, workspace) 的事件去抖后 `refreshStatus()`——**覆盖保存文件（写盘即产生 fs 事件）与外部修改（终端/IDE 改文件）两类场景**。
- **手动刷新兜底**：保留 RefreshCw 按钮——`git status` 关心的部分状态（如终端 commit / stage，只改 `.git` 内部文件）可能不在 fs-watch 覆盖内，手动刷新保证总有出口。
...
- untracked 文件 diff：original 为空串，DiffEditor 呈现全新增。
- workspace 是 repo 子目录时，仅显示 workspace 内变更（§决策 3 范围过滤，已确认）。
- 终端 commit / stage（只改 `.git`）可能不触发 fs-watch → 依赖手动刷新兜底（开放问题 2）。
...
- DiffEditor 的 untracked / binary / no_change 三种特判渲染较细，需单独测试；超大文件（LFS）性能未优化（v1 接受）。
...
| apps/acowork-desktop | `src/lib/workspaceFsWatch.ts` | `deriveWatchGroups` 增加"git 面板展开 → 组内加根路径 ''"派生规则 |
...
- **porcelain 解析器**：空格/中文/引号路径、rename 双路径、`??` untracked、`MM` 双态、submodule 条目。
- **repo discovery**：workspace 即 repo root；workspace 为子目录向上 1~N 层；`.git` 文件（worktree/submodule）；超 6 层返回非 repo。
- **范围过滤**：repo 根在 workspace 外时只返回 workspace 内变更；路径前缀边界（`a/b` vs `a/bc` 不误过滤）。
- **diff 组装**：modified / untracked（original 空）/ binary / no_change 四类；cached=1 的 staged 语义。
- **路径防护**：`../`、绝对路径、符号链接逃逸 → 拒绝。

### 7.2 集成测试（e2e）

- 临时目录 `git init` 造 repo（含 tracked 改动、staged、untracked、rename），起 Runtime 调 `/git/status|diff|log`，断言 JSON/文本；Gateway 反代路径 `GET /api/agents/{id}/git/status` 连通。
- 非 repo workspace → `is_repo: false`；`git` 从 PATH 摘除（测试环境 mock）→ `git_unavailable`。
- 订阅链路：展开 → `PUT /fs-watch` 含根路径；折叠 → 上报后根路径消失；模拟 fs-changed → gitStore 去抖后 status 更新。

### 7.3 Desktop 组件测试

- GitStatusBar 展开/折叠（chevron rotate、面板出现）、计数 pill 渲染；GitStatusPanel 平铺行、状态徽标、行点击打开文件、右键菜单项。
- 虚拟文件：diff tab 渲染 DiffEditor 双栏（original/modified 各就位）、untracked/binary 特判；log tab 只读单栏；readOnly model、关闭释放、不触发保存、不贡献 fs-watch 路径。
...
- **M1**：Runtime `git_query` + 三 API（diff 返回 original/modified 双全文）+ 单元测试（porcelain 解析、repo discovery、范围过滤、路径防护、diff 四类）。
- **M2**：Gateway 反代 + `gitStore` + GitStatusBar/GitStatusPanel 基础（折叠展开、平铺列表、行点击打开文件）+ **订阅 fs-changed（展开订阅/折叠取消）**。
...
2. **终端 commit / stage 的自动刷新**：`git status` 关心的部分状态只改 `.git` 内部文件，可能不被 fs-watch（watch 的是 workspace 根路径）覆盖——手动刷新兜底是否可接受？还是需要额外把 `.git` 加入 watch（有性能/事件噪声顾虑）？
3. **DiffEditor 的 untracked / binary / no_change** 三种特判渲染是否够用？（untracked 显示全新增、binary 显示占位）
4. **repo discovery 上限 6 层**是否合理？workspace 很深时向上搜会不会误命中外层 repo（范围过滤已保证不暴露外层文件，但"误判为 repo"的语义是否可接受）？
5. **分支显示**：`git status --branch` 已含 ahead/behind 信息，但补全计数需要第二条命令——v1 是否只显示分支名（推荐），ahead/behind 留待后续？
6. **DiffEditor 超大/LFS 文件**：`git show HEAD:<path>` 对大文件较慢，v1 是否需要大小上限（如 >2MB 返回 binary 占位）？
