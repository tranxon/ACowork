...
| S3.5.2.3 工具 JSON Schema 清洗 | Schema 格式兼容 LLM |
...
Rollball 开发中优先复用已验证的模式和代码，避免重复造轮子。

**复用原则**：
- 只复用"相对独立"的代码——完整模块、完整函数、边界清晰的 trait
- 需要大幅修改才能适配 Rollball 架构的代码，不直接复用
- 不复用与单进程模式深度耦合的部分（如 Runtime 内嵌的 HTTP Server）

**Phase 1 重点复用领域**：

| 领域 | 参考来源 | Rollball 落地位置 | 说明 |
|------|---------|-----------------|------|
| Tool trait | - | `rollball-core/src/tools/traits.rs` | 直接复用 trait 定义 |
| Provider trait | - | `rollball-core/src/providers/traits.rs` | 直接复用 trait 定义 |
| Schema 清洗 | - | `rollball-runtime/src/tools/schema.rs` | adaptation |
| Streaming 解析 | - | `rollball-runtime/src/providers/` | adaptation |
| Loop Detector | - | `rollball-runtime/src/agent/loop_detector.rs` | adaptation（三种模式 + 三级响应） |
| History Manager | - | `rollball-runtime/src/agent/history.rs` | adaptation（token 计算 + FIFO 裁剪） |
| RateLimitedTool | - | `rollball-runtime/src/tools/wrappers.rs` | adaptation |
| PathGuardedTool | - | `rollball-runtime/src/tools/wrappers.rs` | adaptation |
| JSON-RPC frame | - | `rollball-core/src/protocol.rs` | adaptation |
| Vault 加密 | - | `rollball-vault/src/encryption.rs` | 借鉴加密逻辑 |

**复用要求**：
- 注释标明来源：`// Adapted from <source>` 
- 显著偏离时注明原因：`// Rollball deviation: <reason>`
- 不直接依赖外部 crate，而是 adaptation 后复制到对应 crate

**不直接复用的部分**（需要重新设计）：
- 单一巨型配置文件 → Rollball 按 crate 拆分
- 单进程 Agent Loop → Rollball 需要 IPC 层
- 文件系统沙箱（bubblewrap 集成方式）→ Rollball Phase 3 重新实现
- 配置驱动机制 → Rollball 改用 manifest 声明驱动
