...
> 本目录是 ACowork.AI Gateway 的 API 使用参考。按协议拆分为四份独立文档：
>
> - [HTTP](./http.md) — REST API + WebSocket 升级（现行）
> - [gRPC](./grpc.md) — Gateway ↔ Agent Runtime 的双向流式 IPC（现行）
> - [WebSocket](./websocket.md) — 聊天流式事件通道（现行）
> - [MQTT](./mqtt.md) — 未来架构：实时事件总线 + 轻量状态同步（ADR-033 提案）
>
...
| 协议 | 传输层 | 服务端框架 | 默认端口 | 主要调用方 | 主要用途 | 状态 |
|------|--------|------------|----------|------------|----------|------|
| HTTP/1.1 | TCP | Axum (Rust) | `19876` | Desktop App、CLI、运维脚本 | 资源 CRUD、配置、会话列表、文件浏览等所有非流式场景 | 现行 |
| gRPC | HTTP/2 | Tonic (Rust) | `19877` | Agent Runtime 子进程 | Gateway 与 Runtime 之间唯一的 IPC 通道（双向流式） | 现行 |
| WebSocket | HTTP Upgrade | Axum ws | `19876`（与 HTTP 同端口） | Desktop App（流式 UI） | 聊天的实时事件推送（chunk、工具调用、停止信号等） | 现行 |
| MQTT 3.1.1 | TCP | rumqttd (嵌入) | `19875` | Gateway、Runtime、Desktop Tauri backend | 实时事件总线 + 轻量级状态同步 + 设备生命周期管理（替代 gRPC + WebSocket） | **提案中**（ADR-033） |

> **端口与绑定**：默认全部绑定 `127.0.0.1`（localhost only）。Runtime localhost HTTP server 使用随机端口（`--http-port=0`），仅 Gateway 反向代理可达。可在 `gateway.toml` 的
...
graph LR
    ...
    subgraph GW["Gateway (Axum + Tonic + rumqttd·规划)"]
        HTTP[HTTP API<br/>:19876]
        WS[WebSocket /ws<br/>:19876]
        GRPC[gRPC Server<br/>:19877]
        BROKER[rumqttd Broker<br/>:19875·规划]
    ...
    GW -.->|MQTT PUB/SUB·规划| RT
    UI <-->|MQTT 直连 broker·规划| BROKER
    GW -.->|HTTP 反向代理·规划<br/>(大数据查询)| RT

    classDef ext fill:#eef,stroke:#88a
    classDef gw fill:#efe,stroke:#8a8
    classDef rt fill:#fee,stroke:#a88
    classDef plan fill:#fff5d6,stroke:#cc9,stroke-dasharray: 4 2
    class UI,Script ext
...
4. **（演进）** 未来 [MQTT 架构](./mqtt.md) 把 gRPC + WebSocket 收敛为 MQTT pub/sub + HTTP 反向代理：业务事件直连 broker，状态数据通过 retained 同步，大数据查询通过 Gateway 反向代理到 Runtime localhost HTTP。详见 [ADR-033](../../adr/zh/ADR-033-mqtt-replace-grpc-websocket.md)。
...
当前三种协议（含 Bridge 事件总线）的痛点（手工 `BridgeEventType` 字符串匹配、`GrpcSessionManager` 不可靠的进程感知、多用户需大改路由）在 ADR-033 中已系统论述。**演进目标**：

- **MQTT 替换 gRPC + WebSocket**：Gateway 内嵌 [`rumqttd`](https://github.com/bytebeamio/rumqtt) 作为 broker，Runtime 改为纯 `rumqttc` 客户端 + localhost HTTP server（供 Gateway 反向代理大数据查询），Topic 树天然多租户隔离，Will Message + Retained 解决设备生命周期管理。
- **HTTP 职责不变**：继续承担 CRUD / 全局资源全量管理 / 反向代理到 Runtime HTTP。
- 完整方案：见 [mqtt.md](./mqtt.md)。
...
- gRPC：标准 protobuf
- MQTT（规划）：二进制 protobuf，独立文件 `mqtt_payload.proto`（独立命名空间，不与其他 proto 共享定义）
...
- MQTT（规划）：依赖 localhost-only 绑定；多用户阶段启用 rumqttd 内置 ACL。
...
| 了解 MQTT 架构（ADR-033 演进目标） | [mqtt.md](./mqtt.md) |
...
- MQTT Broker / Publisher（规划）：`core/acowork-gateway/src/mqtt/*.rs`
- Gateway HTTP 反向代理（规划）：`core/acowork-gateway/src/http/proxy.rs`
- Runtime MQTT Client（规划）：`core/acowork-runtime/src/mqtt/client.rs`
- Runtime HTTP Server（规划）：`core/acowork-runtime/src/http/server.rs`
- Proto 定义（gRPC）：`core/acowork-core/proto/gateway_ipc.proto`
- Proto 定义（MQTT，规划）：`core/acowork-core/proto/mqtt_payload.proto`
- 默认端口：`core/acowork-core/src/defaults.rs`
- WebSocket 实现：`core/acowork-gateway/src/http/chat.rs`（`agent_stream_ws` / `handle_ws`）
- ADR-033（MQTT 演进）：[`docs/adr/zh/ADR-033-mqtt-replace-grpc-websocket.md`](../../adr/zh/ADR-033-mqtt-replace-grpc-websocket.md)