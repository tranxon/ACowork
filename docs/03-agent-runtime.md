...
├── Memory Manager      # 记忆生命周期管理（Retrieve/Inject/Record/Consolidate/Decay）
│   ├── Middleware Chain   # 记忆中间件（情感标注/审计/过滤，Phase 2）
│   └── Store Backend      # MemoryStore trait 实现（GrafeoStore / 未来可替换）
├── Grafeo (嵌入式)     # 私有 Memory 存储引擎（GrafeoStore 实现 MemoryStore trait）
...
| 6 | Memory Context | `MemoryManager.retrieve()` + `MemoryManager.inject()` | 记忆检索与注入。通过 MemoryStore trait 的 `hybrid_search` + `graph_expand` 检索，经中间件后处理（Phase 2），按 token 预算裁剪注入。详见 [05-memory.md](./05-memory.md) §10 |
