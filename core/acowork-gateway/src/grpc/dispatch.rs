...
pub async fn dispatch_grpc_request(
    ...
    let response = match client_msg.payload {
        ...
        Some(proto::client_message::Payload::UsageReport(req)) => {
            let report: acowork_core::budget::UsageReport = req.into();
        ...
        Some(proto::client_message::Payload::ContextUsageReport(req)) => {
            let context: acowork_core::protocol::ContextUsageInfo = match req.context {
                Some(c) => c.into(),
                None => acowork_core::protocol::ContextUsageInfo {
                    context_window: 0,
                    input_tokens: 0,
                    output_tokens: 0,
                    total_tokens: 0,
