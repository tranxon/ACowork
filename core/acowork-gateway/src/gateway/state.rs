...
pub struct AgentInfo {
    ...
    /// install time). The install-table key is the instance_id — a
    /// package may be installed multiple times as distinct instances.
...
pub struct RunningAgentInfo {
    /// ADR-073: instance identity — the running-agents table key.
    pub instance_id: String,
    pub agent_id: String,
    pub pid: u32,
...
impl GatewayState {
    ...
    /// Add an installed agent. ADR-073: keyed by instance identity.
    pub fn add_installed(&mut self, info: AgentInfo) {
        // S4.2.2: Register capabilities from manifest on install.
        // ADR-073: keyed by INSTANCE identity — a package installed
        // twice registers two independent capability sets.
    ...
    ///
    /// ADR-073: the install-table key is the INSTANCE identity carried
    /// in the entry; a package may be installed multiple times as
    /// distinct instances.
    ///
