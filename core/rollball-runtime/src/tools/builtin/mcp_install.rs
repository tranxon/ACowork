...
use rollball_core::protocol::{McpServerConfigDef, McpTransportDef};
...
impl Tool for McpInstallTool {
    ...
    async fn execute(
        ...
        // McpServerConfig = McpServerConfigDef (type alias in rollball-mcp),
        // so we can pass the config directly without conversion.
        let scratch_result = rollball_mcp::client::McpClient::connect(new_config.clone()).await;
