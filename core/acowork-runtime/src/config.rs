...
use acowork_core::{AgentInstanceId, Timeouts};
...
pub struct RuntimeConfig {
    /// Agent ID (reverse-domain identifier — PACKAGE identity).
    ///
    /// ADR-073: represents the package only. Used for manifest
    /// cross-validation, logs and envelope payloads. NEVER used for
    /// MQTT topic construction or registry keys — those always use
    /// `agent_instance_id`.
    pub agent_id: String,
    /// ADR-073: instance identity (UUID v4, generated by the Gateway at
    /// install time and injected by the Node at spawn time). REQUIRED
    /// for every Runtime — including standalone / dev invocations; there
    /// is deliberately NO fallback to `agent_id` (the instance id is the
    /// runtime entity's unique identity, independent of standalone
    /// mode). `validate()` rejects an empty value at startup.
    #[serde(default)]
...
impl RuntimeConfig {
    ...
    pub fn from_cli(cli: &Cli) -> Self {
        Self {
            ...
            agent_instance_id: cli.agent_instance_id.clone(),
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        // ADR-073: every Runtime is an instance — a missing or malformed
        // instance id would silently corrupt every MQTT topic this process
        // builds (broker client id, status/config/session/fs topics).
        AgentInstanceId::from_string(self.agent_instance_id.clone()).map_err(|e| {
            format!(
                "--agent-instance-id must be a valid UUID (ADR-073): {e} (got '{}')",
    ...
    /// ADR-073: the instance identity used for MQTT topic construction
    /// and broker client id. Always present in a validated config —
    /// see [`Self::validate`].
...
mod tests {
    ...
    #[test]
    fn validate_accepts_uuid_instance_id() {
        let config = RuntimeConfig {
            agent_instance_id: "6d3f0c2a-9b1e-4c7a-a5d8-2e4f6b8a0c1d".to_string(),
    ...
    fn validate_rejects_missing_instance_id() {
        // ADR-073: an empty instance id must never reach a validated
        // config — every MQTT topic would silently key on nothing.
        let config = RuntimeConfig::default();
        assert!(config.validate().is_err());
    }

    #[test]
    fn validate_rejects_non_uuid_instance_id() {
        // Package ids are NOT instance ids; the old empty-string check
        // accepted them, which corrupted topic construction.
        let config = RuntimeConfig {
            agent_instance_id: "com.test.agent".to_string(),
            ..RuntimeConfig::default()
        };
        assert!(config.validate().is_err());
    }
