...
pub struct AgentHelloConfig {
    ...
    pub search_list: Option<Vec<acowork_core::protocol::SearchProviderListItem>>,
    ...
    pub user_identity: Option<acowork_core::protocol::UserProfile>,
...
/// ADR-021: Single outbound control channel. Data events are no longer
/// sent via gRPC — the frontend polls them via HTTP.
pub struct GatewayGrpcClient {
    /// The endpoint URI (retained for reconnect)
    endpoint: String,
    /// Outbound control sender — exposed via `outbound_ctrl_sender()`
    /// for the chunk relay subsystem.
...
impl GatewayGrpcClient {
    ...
    async fn connect_once(
        ...
        // ADR-021: Single outbound channel for control events.
        let (outbound_ctrl_tx, outbound_ctrl_rx) =
            mpsc::channel::<proto::ClientMessage>(outbound_ctrl_capacity);

        let outbound_stream = ReceiverStream::new(outbound_ctrl_rx);

        // Open bidi-stream RPC
        let response = client
            .connect(outbound_stream)
            .await
            .map_err(|e| AcoworkError::Ipc(format!("gRPC stream establishment failed: {}", e)))?;
    ...
    pub async fn connect(
        ...
        Self::connect_with_timeout(endpoint, 300, outbound_ctrl_capacity).await
    }
    ...
    pub async fn connect_with_timeout(
        ...
        loop {
            match Self::connect_once(endpoint, outbound_ctrl_capacity).await {
                Ok(client) => {
                    tracing::info!(
                        endpoint = %endpoint,
                ...
                Err(e) => {
                    ...
                    if elapsed >= max_elapsed_secs {
                        return Err(AcoworkError::Ipc(format!(
    ...
    pub async fn connect_and_register_with_role(
        ...
        let client = Self::connect(endpoint, outbound_ctrl_capacity).await?;
        let config = client
            .send_agent_hello(
                agent_id,
                version,
    ...
    /// Get a clone of the outbound control message sender.
    ///
    /// ADR-021: Single channel for all control events.
    /// Used by the chunk relay subsystem for Done, Error, Stopped,
    /// SessionStateChanged, and other control events.
    ...
    ) -> Result<proto::ServerMessage, AcoworkError> {
        if !self.is_connected() {
            return Err(AcoworkError::Ipc("Not connected to Gateway".to_string()));
        }
    ...
    pub async fn recv_message(&mut self) -> Result<Option<GatewayResponse>, AcoworkError> {
        ...
        match resp.payload {
            Some(ServerPayload::AgentHelloResult(result)) => {
                if result.success {
                    ...
                    let user_identity: Option<acowork_core::protocol::UserProfile> =
    ...
    pub async fn query_budget(&self, provider: &str) -> Result<(u64, f64), AcoworkError> {
    ...
    pub async fn request_key(&self, provider: &str) -> Result<String, AcoworkError> {
        ...
        match resp.payload {
            Some(ServerPayload::KeyReleaseResult(result)) => {
                if !result.api_key.is_empty() && result.error.is_empty() {
                    ...
                    Err(AcoworkError::Ipc(result.error))
                } else {
                    Err(AcoworkError::Ipc(
    ...
    pub async fn flush_pending_reports(&self) -> Result<usize, AcoworkError> {
    ...
    pub async fn acquire_rate_token(
        &self,
        provider: &str,
    ) -> Result<(bool, Option<u64>), AcoworkError> {
    ...
    pub async fn query_capabilities(
        &self,
        agent_id: Option<&str>,
    ) -> Result<std::collections::HashMap<String, Vec<String>>, AcoworkError> {
    ...
    pub async fn register_cron(
        ...
        action: &str,
        params: serde_json::Value,
    ) -> Result<Result<String, String>, AcoworkError> {
    ...
    pub async fn unregister_cron(&self, cron_id: &str) -> Result<bool, AcoworkError> {
    ...
    pub async fn list_cron(
        &self,
    ) -> Result<Vec<acowork_core::protocol::CronEntryInfo>, AcoworkError> {
    ...
    pub async fn list_sessions(
        &self,
    ) -> Result<Vec<acowork_core::protocol::SessionInfoDto>, AcoworkError> {
    ...
    pub async fn get_session_messages(
        ...
        (
            Vec<acowork_core::protocol::ConversationEntryDto>,
            Option<String>,
            bool,
        ),
        AcoworkError,
    ...
    pub async fn create_session(&self) -> Result<String, AcoworkError> {
...
fn proto_to_gateway_response(msg: proto::ServerMessage) -> GatewayResponse {
    match msg.payload {
        ...
        Some(ServerPayload::RuntimeConfigUpdate(rcu)) => {
            ...
            let mcp_servers: Option<Vec<acowork_core::protocol::McpServerConfigDef>> =
                if rcu.mcp_servers_set {
                    // Sender explicitly set MCP servers, even if the list is empty
                    let parsed: Vec<acowork_core::protocol::McpServerConfigDef> = rcu
                        .mcp_servers_json
                        .iter()
        ...
        Some(ServerPayload::AgentHelloResult(r)) => {
            ...
            let search_list: Option<Vec<acowork_core::protocol::SearchProviderListItem>> =
                if r.search_list_json.is_empty() {
                    None
                } else {
                    serde_json::from_str(&r.search_list_json).ok()
                };
            let search_key_vault: Vec<acowork_core::protocol::SearchKeyEntry> =
