...
pub struct Cli {
    ...
    ///
    /// ADR-073: PACKAGE identity only — used for manifest cross-check,
    /// logs and envelope payloads. NOT a runtime instance identifier;
    /// prefer `--agent-instance-id` for addressing.
    #[arg(long, env = "ACOWORK_AGENT_ID")]
    pub agent_id: String,

    /// ADR-073: instance identity (UUID v4, REQUIRED). Generated by the
    /// Gateway at install time and injected by the Node at spawn time.
    /// The Runtime uses it to construct every `acowork/agents/{instance_id}/...`
    /// topic and its broker client id; it is immutable across node
    /// migration. There is no standalone fallback to `--agent-id` — every
    /// Runtime process is an instance.
    #[arg(long, env = "ACOWORK_AGENT_INSTANCE_ID")]
