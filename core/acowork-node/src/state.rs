...
pub struct AgentSlot {
    /// Instance identity (ADR-073): the runtime entity this slot
    /// represents. The process table key is the instance_id.
...
pub struct InstalledAgent {
    /// Instance identity (ADR-073): UUID v4 generated by the Gateway at
    /// install time. The install table key is the instance_id.
    pub instance_id: String,
    /// Package identity — which agent package this instance was
    /// installed from.
    pub agent_id: String,
    pub version: String,
    pub name: String,
...
pub struct NodeState {
    ...
    pub max_agents: u32,
    /// Runtime process table (running instances, `instance_id` → slot).
    /// ADR-073: key is the instance identity, never the package id.
    pub agents: HashMap<String, AgentSlot>,
    /// Local install table (installed instances, `instance_id` → info).
    /// ADR-073: key is the instance identity, never the package id.
...
impl NodeState {
    ...
    /// `instance_id` is the install-table key (ADR-073).
    pub fn is_installed(&self, instance_id: &str) -> bool {
        self.installed_agents.contains_key(instance_id)
    }

    /// Whether an agent instance process is tracked as running on this
    /// node. `instance_id` is the process-table key (ADR-073).
    pub fn is_running(&self, instance_id: &str) -> bool {
        self.agents.contains_key(instance_id)
    }

    /// Add an installed agent instance to the local install table.
    /// Keyed by the instance identity (ADR-073).
    pub fn add_installed(&mut self, info: InstalledAgent) {
        self.installed_agents.insert(info.instance_id.clone(), info);
    }

    /// Remove an installed agent instance from the local install table.
    pub fn remove_installed(&mut self, instance_id: &str) -> Option<InstalledAgent> {
        self.installed_agents.remove(instance_id)
    }

    /// Add a running agent slot to the process table.
    /// Keyed by the instance identity (ADR-073).
    pub fn add_agent(&mut self, slot: AgentSlot) {
        self.agents.insert(slot.instance_id.clone(), slot);
    }

    /// Remove a running agent slot from the process table.
    pub fn remove_agent(&mut self, instance_id: &str) -> Option<AgentSlot> {
