  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden bg-white dark:bg-zinc-900">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {/* Full content */}
        <div className="mb-4">
          <h3 className="mb-1 text-xs font-medium text-zinc-500 dark:text-zinc-400">Content</h3>
          <p className="whitespace-pre-wrap text-sm text-zinc-800 dark:text-zinc-200">{node.content}</p>
        </div>
        ...
        <div className="mb-4">
          <div className="mb-1 flex items-center justify-between">
            <h3 className="text-xs font-medium text-zinc-500 dark:text-zinc-400">Decay Score</h3>
            <span
              className={cn(
                "text-xs font-medium",
      ...
      <div className="border-t border-zinc-200 p-4 dark:border-zinc-800">
        <button
          ...
          className="inline-flex w-full items-center justify-center gap-1.5 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-xs font-medium text-red-700 hover:bg-red-100 dark:border-red-900 dark:bg-red-950 dark:text-red-400 dark:hover:bg-red-900"
        >
...
function MetaItem({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <p className="mt-0.5 text-xs text-zinc-700 dark:text-zinc-300">{value}</p>
    </div>
  );
}
