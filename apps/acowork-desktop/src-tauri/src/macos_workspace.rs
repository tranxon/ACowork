...
//! returns. `block2::ConcreteBlock::new(...).copy()` is the canonical
//! idiom for that.
//!
//! # Failure modes
//!
...
pub fn install() {
    ...
    let install_result = autoreleasepool(|_| -> Result<(), String> {
        ...
        unsafe {
            ...
            //
            // `ConcreteBlock::new` takes a `'static` closure, and
            ...
            let handler = block2::ConcreteBlock::new(move |_notif: *mut AnyObject| {
