...
#
# vscode-css-languageserver (old npm package) is deprecated and removed.
...
$LegacyBinary = "vscode-css-languageserver"
...
function Find-NpmGlobalBin {
    param([string]$Name)
    $npmBin = "$env:APPDATA\npm"
    # Check both new and legacy binary names
...
function Install-Server {
    ...
    # This single package provides CSS, HTML, JSON, and ESLint servers.
...
# vscode-css-language-server requires --stdio flag.
function Health-Check {
    ...
    try {
        ...
        # Determine which binary to use (new or legacy)
        $testBinary = $Binary
...
# -- Main --------------------------------------------------------------
Write-Host "=== RollBall LSP Setup: vscode-css-language-server ==="
Install-Server
Verify-Server
Health-Check
Write-Host "=== Done ==="
