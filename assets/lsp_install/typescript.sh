...
install() {
    ...
    if command -v "$BINARY" &>/dev/null; then
        echo "typescript-language-server already on PATH at $(command -v "$BINARY")"
        return 0
    fi
    ...
    if [[ -n "$found" ]]; then
        local dir
        dir=$(dirname "$found")
        echo "Found typescript-language-server at $found — adding $dir to PATH..."
        add_to_path "$dir"
        return 0
    fi
...
verify() {
    ...
    if command -v "$BINARY" &>/dev/null; then
        echo "OK: typescript-language-server found at $(command -v "$BINARY")"
        return 0
    fi
